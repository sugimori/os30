...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1) {
  ...
  for (h = h0; h <= h1; h++) {
    ...
    if ((sht->vx0 & 3) == 0) {
      // 4バイト版
      i = (bx0 + 3) / 4;  // bx0を４で割ったもの（端数切り上げ）
      i1 = bx1 / 4;       // bx1を４で割ったもの（端数切捨て）
      i1 = i1 - i;
      sid4 = sid | sid << 8 | sid << 16 | sid << 24;
      for (by = by0; by < by1; by++) {
        vy = sht->vy0 + by;
        for (bx = bx0; bx < bx1 && (bx & 3) != 0; bx++) {  // ４の倍数にならない最初の端をを１バイトずつ
          vx = sht->vx0 + bx;
        ...
        for (i = 0; i < i1; i++) {  // ４の倍数部分
          if (p[i] == sid4) {       // 4バイト全部一緒
            q[i] = r[i];
          } else {
            bx2 = bx + i * 4;     // 相対アドレス
            vx = sht->vx0 + bx2;  // vramアドレス
...
void sheet_slide(struct SHEET *sht, int vx0, int vy0) {
  ...
  sht->vy0 = vy0;
  if (sht->height >= 0) {
    sheet_refreshmap(ctl, old_vx0, old_vy0, old_vx0 + sht->bxsize, old_vy0 + sht->bysize,
                     0);  // 移動したから全部書き換え
    sheet_refreshmap(ctl, vx0, vy0, vx0 + sht->bxsize, vy0 + sht->bysize, sht->height);

    sheet_refreshsub(ctl, old_vx0, old_vy0, old_vx0 + sht->bxsize, old_vy0 + sht->bysize, 0,
                     sht->height - 1);  // 移動したから全部書き換え
