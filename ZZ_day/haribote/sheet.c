...
void sheet_slide(struct SHEET *sht, int vx0, int vy0)
{
    ...
    sht->vy0 = vy0;
    if(sht->height >= 0) {
        sheet_refreshmap(ctl, old_vx0, old_vy0, old_vx0 + sht->bxsize, old_vy0 + sht->bysize,0); // 移動したから全部書き換え
        sheet_refreshmap(ctl, vx0, vy0, vx0 + sht->bxsize, vy0 + sht->bysize, sht->height);

        sheet_refreshsub(ctl, old_vx0, old_vy0, old_vx0 + sht->bxsize, old_vy0 + sht->bysize,0, sht->height-1); // 移動したから全部書き換え
